...
> **Version:** 1.10.0 · **Site:** [cportka.github.io/claude-plugins](https://cportka.github.io/claude-plugins/) · **License:** [MIT](./LICENSE) · **Changelog:** [CHANGELOG.md](./CHANGELOG.md) · **Roadmap:** [IMPROVEMENTS.md](./IMPROVEMENTS.md) · **Maintainers:** [docs/HANDOFF.md](./docs/HANDOFF.md) · **Privacy:** [PRIVACY.md](./PRIVACY.md) · **Security:** [SECURITY.md](./SECURITY.md)
...
`--ocr-roi`, `--measure`, `--probe`, `--palette` (+ `--over-time`), `--loop-check`,
`--ab`/`--compare-videos`, `--stutter` (FPS stutter, dropped frames + freeze gaps; add
`--marks perf.json` to correlate freezes with your app's own `performance.mark` data), `--pacing`,
`--motion`, `--flow`, `--occupancy`, `--saturation`, and the `--intro` load-bug preset — each
documented in `--help` and the skill's `reference.md`.
